...
GameCoach AI 是一个面向游戏玩家的智能成长教练 Agent。系统通过 LangGraph 构建多 Agent 工作流，结合 Planning、Tool Calling、RAG、Memory 与 LangSmith 监控能力，帮助玩家分析近期战绩、定位能力短板、推荐角色与出装，并生成可执行的阶段性训练计划。
...
- 使用 Tool Calling 接入战绩、角色数据库、版本信息和攻略检索工具。
...
[
  ...
  {
    "task": "character_pool_analysis",
    "description": "分析玩家常用角色、胜率较高角色和低效角色"
  },
  {
    "task": "version_recommendation",
    "description": "结合当前版本强势角色，推荐适合玩家上分的角色"
  },
  {
    "task": "training_plan",
...
| `build_agent` | 推荐角色构筑、装备和技能加点 | 角色、版本、玩家风格 | 构筑推荐 |
...
- 大多数玩家请求意图明确（"分析战绩""推荐角色""生成计划"），不需要边执行边调整方向
...
- 理解用户自然语言意图。
- 判断用户是否需要战绩分析、角色推荐、出装推荐、攻略检索或成长计划。
- 生成结构化任务列表。
- 为每个任务标注优先级和依赖工具。
...
```json
{
  ...
  "planned_tasks": [
    ...
    {
      "task_id": "t2",
      "task_type": "character_recommendation",
      "description": "根据角色池和当前版本推荐上分角色",
      "priority": 2,
...
```json
{
  ...
  "strengths": ["对线补刀稳定", "使用damage role角色熟练度较高"]
}
```
...
```json
{
  "character": "Bravo",
  "scenario": "均势局",
  "items": ["Basic_Boots", "Core_Item", "Crit_Core", "Pen_Item", "Range_Bow", "Magic_Resist"],
...
```json
{
  ...
  "favorite_characters": ["Alpha", "Bravo"],
  "main_roles": ["damage role"],
...
| `Build` | 建议"带什么" | 出装顺序、modifier搭配 | 枪械配件、道具组合 | 卡组构筑、sideboard |
| `Memory` | 记住"玩家是谁" | 偏好角色、主玩位置 | 偏好武器、主玩地图 | 偏好职业/卡组类型 |

Agent 的**角色不变**（诊断/建议/记忆），但调用的 Tool 和注入的领域知识随游戏类型切换。因此每个 Agent 不应硬编码"damage role""KDA"等 MOBA 概念，而应通过游戏配置抽象为"角色""核心指标"。详见 §12.1 可扩展性设计。
...
| --- | --- | --- |
| 关注点 | 决策、时机、站位、节奏 | 装备、modifier、天赋、构筑 |
| 示例 | "团战站后排，等刺客先手" | "出Core_Item → 无尽 → Pen_Item" |
| 示例 | "10 分钟后别单带过河道" | "对面法刺多，逐日换魔女" |
| 示例 | "逆风只吃安全线" | "顺风局先出输出，逆风先出肉" |

类比：Strategy = 驾校教练教你怎么开车（什么时候变道、怎么判断车距），Build = 技师告诉你这辆车用什么轮胎、加什么油。两者互补：先有 Strategy（"我需要容错率高的打法"），再有 Build（"那就出Magic_Resist保命"）。在无装备系统的游戏（如部分 FPS）中，Strategy 可独立工作，Build Agent 可被跳过。
...
| --- | --- | --- |
| 定位 | 决策层："大脑" | 存储层："数据库" |
| 职责 | 从对话和分析结果中提取长期偏好；判断是否更新目标、弱点、角色池 | JSON 文件读写、Schema 定义、CRUD 接口 |
...
Strategy Agent 负责生成阶段性的训练方向（如"先用 2 个容错角色打 20 场"），而具体的每日训练计划（Day 1 补刀、Day 2 小地图…）可以由 Strategy Agent 内部生成，也可以通过独立的 TrainingPlan Agent 生成。MVP 阶段合并在 Strategy Agent 中，进阶阶段可拆分。
...
{
  "patch": "2026.06",
  "strong_characters": ["Bravo", "Bravo", "Charlie"],
  "nerfed_characters": ["Alpha"],
  "buffed_items": ["Range_Bow"],
  "meta_summary": "当前版本damage role更重视自保和中期参团能力。"
}
```
  ...
  "documents": [
    {
      ...
      "title": "damage role团战站位指南",
      "score": 0.87,
      "snippet": "damage role应保持在前排后方，等待敌方关键控制和突进技能交出后再进场输出。"
    }
  ]
}
```
...
交叉验证只对高风险的具体战术建议（如"damage role第一件出 X"）执行。
...
```
检索到的攻略："damage role应该站在前排后方 600-800 距离输出"
模型输出：    "damage role应该主动绕后切对面后排"  ← 完全编的
```
...
class AdviceItem(BaseModel):
    claim: str                          # "Alpha第一件出Basic_Boots"
    source_doc_id: str                  # "character_guide_alpha.md"
    source_snippet: str                 # "Alpha首件推荐Basic_Boots，提升攻速..."
...
```
攻略原文："逆风局damage role不要单带过河道，应抱团清线"
模型曲解为："逆风局damage role应该带线牵制"  ← 理解反了
```
...
```
模型输出："根据《a MOBA gamedamage role进阶指南》第3章..."  ← 这本书不存在
```
...
| 角色/职业攻略 | 半结构化：属性表 + 技能说明 + 出装推荐 + 对局技巧 | 按章节标题切分（`##`），保留表格完整性 | 语义窗口——表格的 embedding 语义弱，容易丢 |
| 职业复盘/教学 | 长文叙事，逻辑流强 | 语义窗口（512 token，overlap 128），按段落边界调整 | 固定大小——会切断推理链 |
| 装备/modifier解析 | 短条目，每条独立 | 最小单元切分（每条装备一个 chunk），保留完整字段 | 任何切分——应以结构化 JSON 存而非向量检索 |
...
class ChunkingStrategy:
    STRATEGIES = {
        ...
        "character_guide":     SectionalChunker(heading_level=2, min_tokens=300, max_tokens=1200),
...
3. **结构化数据不进向量库**。角色属性表、装备数值等精确查询走 Tool Calling（`character_database_tool`），而非向量检索。向量检索适合"damage role怎么站位"这类开放问题，不适合"Alpha攻击力多少"这类精确查询。
...
- 玩家目标：例如上钻石、提高胜率、练会off-role。
- 当前弱点：例如团战意识差、补刀差、角色池浅。
- 玩家可投入时间：例如每天 30 分钟。
- 当前版本推荐方向。
...
> **状态更新 (2026-06-13)**：Milestone 1-5 已完成，Milestone 6 完成 CLI + Web 双入口。

### Milestone 1: 项目骨架 ✅

- [x] 初始化 Python 3.11 项目。
- [x] 配置 `pyproject.toml`。
- [x] 建立 `src/gamecoach` 目录。
- [x] 定义 `GameCoachState`。
- [x] 准备 Mock 数据。

### Milestone 2: LangGraph MVP ✅

- [x] 实现 Planner Agent（LLM 驱动 + 规则 fallback）。
- [x] 实现 Task Router（条件路由 DAG）。
- [x] 实现 MatchAnalysis Agent。
- [x] 实现 Response Agent。
- [x] 跑通一次完整工作流。

### Milestone 3: Tool Calling ✅

- [x] 实现战绩查询 Tool（`@tool` 封装）。
- [x] 实现角色数据库 Tool（`@tool` 封装）。
- [x] 实现版本信息 Tool（`@tool` 封装）。
- [x] 实现出装查询 Tool（`@tool` 封装）。
- [x] 增加 Tool 错误处理。
- [x] 编写工具单元测试（8 个）。

### Milestone 4: Memory 与 RAG ✅

- [x] 实现 Player Memory JSON 存储（读写 + 两层记忆模型）。
- [x] 接入攻略文档加载与 FAISS 向量检索。
- [x] 实现 RAG Agent。
- [x] 将 Memory 和 RAG 结果接入 Strategy Agent。
- [x] 5 篇中文攻略文档。
- [x] Embedding 使用阿里 DashScope (text-embedding-v2)。

### Milestone 5: 评估与 LangSmith ✅

- [x] 接入 LangSmith Trace（通过环境变量配置）。
- [x] 增加 Agent 执行指标（路由路径、降级节点、RAG 命中、回复长度）。
- [x] 实现基础评估脚本 (`evaluation/metrics.py`)。
- [x] 17 个单元/集成测试全部通过。

### Milestone 6: Demo 展示 ✅

- [x] CLI Demo（`python -m gamecoach.main --demo 0`）。
- [x] Web Demo（`uvicorn gamecoach.api:app`）。
- [x] 4 个预设演示场景。
- [x] 完整项目文档。
...
```text
基于 LangGraph 构建多 Agent 游戏成长教练系统，面向玩家战绩分析、角色推荐、出装建议、攻略检索和训练计划生成等场景。系统通过 Planner Agent 拆解用户意图，由 Task Router 调度 MatchAnalysis、Strategy、Build、RAG 与 Memory 等专用 Agent，并结合 Tool Calling 获取战绩、角色数据库和版本信息。项目引入 Player Memory 记录玩家角色偏好、能力短板和成长目标，使用 RAG 检索职业攻略与版本指南，最终生成个性化上分建议和阶段训练计划。通过 LangSmith 监控任务拆解正确率、工具调用成功率、RAG 命中率与平均响应时间，并设计胜率、KDA、段位变化和任务完成率等指标评估建议效果。
```
...
- 战绩查询、角色数据库、版本信息至少使用 Mock Tool 跑通。
- Memory 能读取和更新玩家画像。
- RAG 能从本地攻略文档中检索相关内容。
...
```text
玩家某次说了句"我觉得我off-role还行"
  → Memory Agent 写入：strengths += "off-role"
  → 后续 Planner 看到 memory：这个玩家擅长off-role
  → 推荐的角色和策略都偏向off-role
  → 玩家的实际问题是damage role位，off-role建议全部无效
  → 但系统已经"相信"这个玩家喜欢off-role了
```
...
class MemoryUpdatePolicy:
    ...
    def should_update(self, key: str, value: str, source: str, evidence_count: int) -> bool:
        source_weight = {
            ...
            "explicit_stated":  0.7,   # 玩家明确说"我只玩damage role" → 中高置信
...
```text
一个低质量攻略进了知识库："Alpha第一件出破军，伤害爆炸"
  → 玩家问"Alpha怎么出装" → RAG 检索到这个攻略
  → 系统推荐破军首件 → 玩家反馈：输了
...
Planner 一次错误拆解（如用户问"怎么上分"被错误拆出"先练off-role角色"）会导致整个链路基于错误假设运行。
...
class LLMSanitizer:
    ...
    ALLOW_FIELDS = {
        ...
        "favorite_characters", "main_roles", "weaknesses",
        "goals", "rank", "match_metrics", "character_win_rates",
    }
...
**关键设计**：LLM 看到的"玩家"是一个匿名实体——有段位、有角色偏好、有战绩指标，但不知道是谁。这足够生成建议，且零 PII 泄露。
...
| 环境 | 策略 |
| --- | --- |
| 本地开发 | 全部 Mock 数据（`player_001`、`Alpha`、`铂金 I`），不存在真正的敏感信息 |
